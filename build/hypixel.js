...
async function cleanResponse({ path, data }, options) {
    ...
    switch (path) {
        ...
        case 'skyblock/auctions': return await (0, auctions_1.cleanSkyBlockAuctionsResponse)(data);
        case 'skyblock/auctions_ended': return await (0, auctions_1.cleanSkyBlockAuctionsResponse)(data);
    }
}
/**
...
async function getItemLowestBin(item) {
    var _a;
    ...
    const enchantments = (_a = item.enchantments) !== null && _a !== void 0 ? _a : {};
    ...
    if (item.id === 'ENCHANTED_BOOK' && Object.keys(enchantments).length === 1) {
        ...
        const matchingEnchantedBooks = matchingBins.filter(a => {
    ...
    for (const auction of matchingBins) {
        ...
        for (const enchantment of auctionMissingEnchantments) {
            ...
            auctionEnchantmentsPrice += Math.max(enchantmentValue - 10000, 0);
        }
...
exports.getItemLowestBin = getItemLowestBin;
async function getEnchantmentBinPrice(enchantmentName, enchantmentLevel) {
    const enchantments = {};
