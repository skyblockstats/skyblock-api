...
export async function getItemUniqueId<U extends boolean>(item: Item, update: U): Promise<U extends true ? string : string | undefined> {
	...
	if (!update) return existingItem?._id as any
...
async function addEndedAuctions() {
	// we don't use https://api.hypixel.net/skyblock/auctions_ended because it seems to only return bin items
	...
	for (const auction of endedAuctions) {
		...
		if (!previouslyEndedAuctionIds.includes(auction.uuid) && !auction.bin && auction.bidAmount) {
